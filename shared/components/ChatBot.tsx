...
import { GEMINI_API_KEY, PYTHON_BACKEND_URL, isGeminiEnabled, isPythonBackendEnabled } from '@/config/env';
...
const SYSTEM_PROMPT = `You are a helpful financial AI assistant for AuraFinance, a comprehensive financial platform. Your role is to:
...
export function ChatBot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: 'Hello! I\'m your AuraFinance AI assistant. How can I help you with your financial needs today?',
      timestamp: Date.now()
    }
  ]);
      ...
      if (isPythonBackendEnabled()) {
        try {
          const fetchContextChunks = async (query: string): Promise<string[]> => {
            const res = await fetch(`${PYTHON_BACKEND_URL}/chroma-search?q=${encodeURIComponent(query)}`);
