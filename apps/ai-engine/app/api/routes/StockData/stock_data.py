...
async def get_stock_quote(symbol: str):
    ...
    try:
        ...
        change = safe_float(current_price - previous_close)
        change_percent = safe_float((change / previous_close) * 100 if previous_close else 0)
        ...
        return {
            ...
            "fiftyTwoWeekHigh": safe_float(info.get('fiftyTwoWeekHigh', 0)),
            "fiftyTwoWeekLow": safe_float(info.get('fiftyTwoWeekLow', 0)),
    ...
    try:
        ...
        for record in data:
            formatted_record = {
                ...
                "open": round(safe_float(record['open']), 2),
                "high": round(safe_float(record['high']), 2),
                "low": round(safe_float(record['low']), 2),
                "close": round(safe_float(record['close']), 2),
                "volume": int(safe_float(record['volume']))
            }
